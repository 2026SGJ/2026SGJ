...
class MatchManager {
	...
	addBot(team) {
		...
		const bot = new BotPlayer(botId, {
			...
			hero: botHero,
			name: botName,
			// 人机补位同样随机部署一个 AI 机器人（机器人 ≠ 玩家）
			robotType: pickRandomRobotType(),
		});
		// 行动许可跟随阶段：匹配中仅能移动，对局中可完全行动
		bot.canAct =
			this.phase === Phase.PLAYING || this.phase === Phase.SUDDEN_DEATH;
		this.game.players[botId] = bot;
		// 对局中补位的人机立即部署 AI 机器人（匹配阶段由 _startGame 统一部署）
		if (this.phase === Phase.PLAYING || this.phase === Phase.SUDDEN_DEATH) {
			this.game.robotManager.spawnFor(bot);
		}
		console.log(
			`[Match] 人机加入: ${botId} → ${chosenTeam} 队（当前共 ${this.totalPlayers()} 人）`,
		);
	...
	kickBot(preferredTeam) {
		const bots = Object.keys(this.game.players).filter((id) =>
			id.startsWith(BOT_PREFIX),
		);
		if (bots.length === 0) return false;

		const target =
			bots.find((id) => this.game.players[id].team === preferredTeam) ||
			bots[0];
		const bot = this.game.players[target];
		delete this.game.players[target];
		// 清理该人机的 AI 机器人（机器人跟随玩家归属）
		this.game.robotManager.removeRobotFor(target);
	...
	_tickBases() {
		for (const base of this.game.world.bases) {
			...
			for (const p of Object.values(this.game.players)) {
				...
				if (
					Math.hypot(p.x - base.data.x, p.y - base.data.y) > BASE_DAMAGE_RADIUS
				)
					continue;
				attackers++;
			}
	...
	_finish(result) {
		...
		// AI 机器人同样停止行动（机器人 ≠ 玩家，但共享阶段控制）
		this.game.robotManager.stopAll();
	...
	_startGame() {
		...
		// 部署 AI 机器人：每名玩家进局前 5 选 1（未选则随机）
		// 机器人 ≠ 玩家：不进 players，不参与复活 / 占领 / 胜负结算
		this.game.robotManager.spawnAll();
	...
	/** 房间内人机数量 */
	botCount() {
		return Object.keys(this.game.players).filter((id) =>
			id.startsWith(BOT_PREFIX),
		).length;
	}
	...
	_broadcastProgress() {
		...
		if (!this.countdownStarted) {
			...
			remaining = Math.max(
				0,
				Math.ceil((this.matchStartAt - Date.now()) / 1000),
			);
