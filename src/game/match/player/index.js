...
import TraitManager from "../trait/TraitManager.js";
 ...
 * 深拷贝英雄配置（按玩家独立一份）
 *
 * 词条会修改技能数值（伤害 / 冷却 / 持续等），若所有同英雄玩家共享 HERODATAS
 * 配置对象，一个玩家的词条会污染其他同英雄玩家，且商店永久加速等也受影响。
 * 此处构造时即为每个玩家深拷贝一份英雄数据（数据为纯 JSON，structuredClone 安全）。
 */
const deepCloneHeroData = (data, heroId) => {
	if (!data) {
		throw new Error(`Hero data not found for hero: ${heroId}`);
	}
...
class Player {
	...
	constructor(sessionId, data) {
		...
		/**
		 * 玩家进局前选择的 AI 机器人兵种（null = 未选，对局开始时由 RobotManager 部署）
		 * 注意：AI 机器人 ≠ 人机补位（BotPlayer）；玩家拥有并指挥，但机器人不视为玩家
		 * @type {string|null}
		 */
		this.robotType = data.robotType || null;
		this.costume = "empty";
		this.runAnimate = 0;
		...
		// ---------- 词条系统 ----------
		// 每玩家独立词条管理器（人机同样拥有，见 BotController）
		/** @type {import('../trait/TraitManager.js').default} */
		...
		// ---------- 词条管理器（须在 skills 创建后初始化，词条会重建技能实例） ----------
	...
	animate() {
		...
		this.runAnimate =
			...
			this.args.animations.run.frames;
		...
		if (typeof singleBtn === "string" && singleBtn in state.buttons) {
			...
			const pressed =
				data.pressed !== undefined
					? !!data.pressed
					: data.type === "GamepadDown";
			state.buttons[name] =
				name === "lt" || name === "rt" ? (pressed ? 1 : 0) : pressed;
		}
	...
	_normalizeTouchEvent(data) {
		...
		if (data.virtual === true) {
			...
			if (type === "joystick") {
				...
				target.active =
					pressed && Math.hypot(target.x, target.y) > this._deadzone;
			} else if (control) {
		...
		// ---- 快照型 ----
		if (
			data.type === "TouchHolding" ||
			data.type === "TouchState" ||
			data.joystick !== undefined ||
			data.aim !== undefined ||
			data.buttons ||
			...
			if (data.joystick !== undefined) {
				if (data.joystick) {
					...
					state.joystick.active =
						Math.hypot(state.joystick.x, state.joystick.y) > this._deadzone;
				} else {
					// 显式 null = 松开摇杆
					state.joystick.x = 0;
					state.joystick.y = 0;
			...
			if (data.aim !== undefined) {
				if (data.aim) {
					...
					state.aim.active =
						Math.hypot(state.aim.x, state.aim.y) > this._deadzone;
				} else {
					state.aim.x = 0;
					state.aim.y = 0;
			...
			if (data.click !== undefined) {
				state.click = data.click
					? {
							x: Number(data.click.x),
							y: Number(data.click.y),
							world: !!data.world,
						}
					: null;
			}
	...
	updateOutpostProximity(world) {
		const nearbyOutpost = world.getNearbySpawnOutpost(
			this.x,
			this.y,
			this.team,
		);
		...
		if (
			this.customSpawnOutpost &&
			!this.customSpawnOutpost.isSpawnValid(this)
		) {
			console.log(
	...
	move(world) {
		...
		if (this.dx && kb <= 16)
			this.speed.x = this.dx * this.args.speed * speedMult;
		if (this.dy && kb <= 16)
			this.speed.y = this.dy * this.args.speed * speedMult;
	...
	/**
	 * 查找最近的低血量敌人（75px 范围内，词条可扩展 attackRange）
	 * 优先血量最低者，同等血量优先距离最近者
	 * （跳过已死亡玩家，目标含敌方 AI 机器人）
	 * @param {Object<string, import('../player/index.js').default>} players
	 * @param {import('../robot/RobotManager.js').default|null} [robots] — AI 机器人管理器
	 */
	findTarget(players, robots) {
		let bestTarget = null;
		let bestHealth = Infinity;
		let bestDistance = Infinity;
		// 攻击判定距离（词条可扩展，如高斯·磁轨加速 +20% 射程）
		const range = this.attackRange || 75;
		...
		for (const [id, player] of Object.entries(players)) {
			...
			if (
				health < bestHealth ||
				(health === bestHealth && dist < bestDistance)
			) {
				bestTarget = player;
				bestHealth = health;
				bestDistance = dist;
		...
		// AI 机器人也可被玩家攻击（机器人不是玩家，但不免疫伤害）
		if (robots && typeof robots.all === "function") {
			for (const r of robots.all()) {
				if (r.team === this.team) continue; // 忽略己方机器人
				if (!r.alive || r.hp <= 0) continue;
				const dist = Math.hypot(this.x - r.x, this.y - r.y);
				if (dist > range) continue;
				if (r.hp < bestHealth || (r.hp === bestHealth && dist < bestDistance)) {
					bestTarget = r;
					bestHealth = r.hp;
					bestDistance = dist;
				}
	...
	/**
	 * 查找范围内所有敌人（用于 AOE，跳过已死亡玩家，含敌方 AI 机器人）
	 * @param {Object<string, import('../player/index.js').default>} players
	 * @param {number} range - 溅射范围（像素）
	 * @param {import('../robot/RobotManager.js').default|null} [robots] — AI 机器人管理器
	 */
	findTargetsInRange(players, range, robots) {
		const targets = [];
		for (const [id, player] of Object.entries(players)) {
			if (id === this.sessionId) continue;
			if (player.team === this.team) continue; // 忽略队友
	 ...
	 * 处理普攻
	 * 前摇计时 → 计时归零时命中目标
	 * 匹配阶段禁止攻击。目标可为敌方玩家或敌方 AI 机器人。
	 */
	processBasicAttack(players, robots) {
		...
		if (this.attackForward <= 0) {
			...
			if (target) {
				...
				// 词条钩子：普攻命中（如牛顿·质量守恒获得护盾）
				this.trigger("trait:basicHit", { target });
	 ...
	 * 处理技能释放
	 * 前摇计时 → 计时归零时释放技能效果
	 * 匹配阶段禁止释放技能。目标可为敌方玩家或敌方 AI 机器人。
	 */
	processSkillCast(players, robots) {
		...
		if (this.skillCastForward <= 0) {
			...
			if (skillData.magic && skillData.magic.range) {
				const aoeTargets = this.findTargetsInRange(
					players,
					skillData.magic.range,
					robots,
				);
			...
			// 词条钩子：技能释放（如特斯拉·超导爆发附加眩晕）
			this.trigger("trait:skillUsed", { skillKey, skillData, target });
		}
	}
	 ...
	 * @param {import('../robot/RobotManager.js').default|null} [robots] — AI 机器人管理器
	 */
	processSkills(players, robots) {
		this.processBasicAttack(players, robots);
		this.processSkillCast(players, robots);
	}
	...
	remoteData() {
		...
		return {
			...
			state: {
				...
				spawnOutpostId: this.customSpawnOutpost
					? this.customSpawnOutpost.data.id
					: null,
				...
				// 词条：已获得词条列表（id / 名称 / 稀有度，供客户端展示）
				traits: this.traitManager ? this.traitManager.serialize() : [],
				channelingTeleport: this.inventory
					? this.inventory.isChannelingTeleport
					: false,
				teleportRemaining: this.inventory
					? this.inventory.teleportChannelRemaining
					: 0,
			},
			fz: 1,
			"z-index": 1000,
	 ...
	 * @param {import('../robot/RobotManager.js').default|null} [robots] — AI 机器人管理器（供攻击目标查询）
	 */
	tick(players, world, robots) {
		this.dx = this.dy = 0;
	 ...
	 */
	render(f) {
		const entities = f(this.x, this.y, 320, 180);
		const renderData = [];
	...
	takeDamage(amount, attacker) {
		...
		// ----- 词条概率减伤（如特斯拉·绝缘涂层 20% 免疫 30%） -----
		if (this._traitDmgReductions && finalAmount > 0) {
			for (const r of this._traitDmgReductions) {
				if (Math.random() < r.chance) {
					finalAmount = Math.round(finalAmount * (1 - r.reduce));
				}
			}
		}
	...
	onDeath() {
		...
		if (!this.canRevive) {
			...
			console.log(
				`Player ${this.sessionId} 无法复活（基地被毁或加时赛），保持阵亡状态`,
			);
			return;
		}
	...
	giveBuff(buff) {
		// 词条：控制时间缩减（如牛顿·绝对惯性，stun/freeze 类减益持续时间按比例缩短）
		if (
			this._ccReduction > 0 &&
			(buff.id === "stun" || buff.id === "freeze") &&
			buff.duration > 0
		) {
			buff.duration = Math.max(
				1,
				Math.round(buff.duration * (1 - this._ccReduction)),
			);
		}
	...
	useItem(itemId, options = {}) {
		...
		if (!this.inventory.usesCooledDown(itemId)) {
			...
			console.log(
				`[ItemUse] ${this.sessionId}: 道具 ${config.name} 冷却中, 剩余 ${remaining}ms`,
			);
			return false;
		}
		...
		switch (config.type) {
			case "consumable":
				success = this._useConsumable(config);
				break;
			case "placeable":
				success = this._usePlaceable(config, effectiveWorld);
				break;
			case "projectile":
				success = this._useProjectile(config, effectiveWorld);
				break;
			case "utility":
				success = this._useUtility(config);
				break;
			default:
		...
		if (success) {
			...
			console.log(
				`[ItemUse] ${this.sessionId} 使用了 ${config.name}, 剩余 ${this.inventory.count(itemId)} 个`,
			);
		}
	...
	_useConsumable(config) {
		...
		console.log(
			`[ItemUse] ${this.sessionId} 使用 ${config.name}, 回复 ${this.health - oldHealth} HP (${this.health}/${this.maxHealth})`,
		);
		return true;
	}
	...
	_usePlaceable(config, world) {
		if (!world) {
			console.warn(`[ItemUse] _usePlaceable 需要 world 参数`);
			return false;
		}
		...
		switch (config.id) {
			case "bomb":
				entity = new BombEntity(this.x, this.y, data, this.sessionId);
				break;
			case "landmine":
				entity = new LandmineEntity(
					this.x,
					this.y,
					data,
					this.sessionId,
				...
				break;
			case "fragGrenade":
				entity = new FragGrenadeEntity(this.x, this.y, data, this.sessionId);
				break;
			case "smokeGrenade":
				entity = new SmokeGrenadeEntity(
					this.x,
					this.y,
					data,
					this.sessionId,
			...
			case "healingTotem":
				...
				break;
			default:
				console.warn(`[ItemUse] 未处理的放置物类型: ${config.id}`);
				return false;
		}
		if (entity) {
			world.addItemEntity(entity);
			console.log(
				`[ItemUse] ${this.sessionId} 放置 ${config.name} 于 (${this.x.toFixed(0)}, ${this.y.toFixed(0)})`,
			);
			return true;
		}
		return false;
	...
	_useProjectile(config, world) {
		if (!world) {
			console.warn(`[ItemUse] _useProjectile 需要 world 参数`);
			return false;
		}
		const data = config.data;
		...
		switch (config.id) {
			...
			case "poisonDart":
				...
				break;
			default:
				console.warn(`[ItemUse] 未处理的投射物类型: ${config.id}`);
				return false;
		}
		if (entity) {
			world.addItemEntity(entity);
			console.log(
				`[ItemUse] ${this.sessionId} 发射 ${config.name} 方向 (${direction.x.toFixed(2)}, ${direction.y.toFixed(2)})`,
			);
			return true;
		}
		return false;
	...
	_useUtility(config) {
		...
		switch (config.id) {
			case "teleportScroll": {
				...
				console.log(
					`[ItemUse] ${this.sessionId} 开始引导回城, 需要 ${this.inventory.teleportChannelRemaining}ms`,
				);
				return true;
			}
			case "speedPotion": {
				...
				this.giveBuff(
					new SpeedBuffClass({
						id: "speed",
						level: Math.round(data.speedBoost * 100),
						time: data.duration || 8000,
					}),
				);
				return true;
			}
			case "invisibleCloak": {
				...
				this.giveBuff(
					new InvisibleBuffClass({
						id: "invisible",
						level: Math.round(data.dmgReduction * 100),
						time: data.duration || 4000,
					}),
				);
				return true;
			}
			case "shieldStone": {
				...
				this.giveBuff(
					new ShieldBuffClass({
						id: "shield",
						level: data.shieldAmount || 500,
						time: data.duration || 10000,
					}),
				);
				return true;
			}
			case "thornArmor": {
				...
				this.giveBuff(
					new ReboundBuffClass({
						id: "rebound",
						level: data.reflectPercent || 30,
						time: data.duration || 6000,
					}),
				);
				return true;
			}
	...
	processTeleportChannel() {
		...
		if (inv.teleportChannelRemaining <= 0) {
			...
			this.knockback.set(0, 0);
			console.log(
				`[Teleport] ${this.sessionId} 回城成功 → (${this.x.toFixed(0)}, ${this.y.toFixed(0)})`,
			);
		}
	}
}
