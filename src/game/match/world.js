...
class World {
	...
	async init() {
		...
		try {
			mapData = JSON.parse(
				fs.readFileSync(`./src/game/map/${this.map_id}.json`, "utf-8"),
			);
		} catch (err) {
			// 防御：地图文件缺失 / 损坏时退回空地图，避免服务器启动崩溃（日志可定位问题）
			console.error(
				`[World] 地图加载失败 ./src/game/map/${this.map_id}.json:`,
				err.message,
			);
			mapData = {
				assetId: this.map_id || "1",
		...
		for (const entity of mapData.entities) {
			...
			if (entity.type === "outpost") {
				...
				console.log(
					`[World] 前哨站实体已加载: id=${entity.id}, 位置 (${entity.x}, ${entity.y})`,
				);
			...
			if (entity.type === "shop") {
				...
				console.log(
					`[World] 商店实体已加载: id=${entity.id}, 位置 (${entity.x}, ${entity.y})`,
				);
				continue;
			}
			...
			if (entity.type === "base") {
				...
				console.log(
					`[World] 基地实体已加载: id=${entity.id}, 队伍=${entity.team}, 位置 (${entity.x}, ${entity.y})`,
				);
				continue;
			}
	...
	_addDefaultBases() {
		...
		console.log(
			"[World] 默认基地已创建: A(1280,6840) / B(1280,360)，血量 4000",
		);
	}
	...
	/**
	 * 添加 AI 机器人实体到世界（仅渲染列表；逻辑由 RobotManager 驱动）
	 * @param {import('./robot/RobotEntity.js').default} robot
	 */
	addRobot(robot) {
	...
	/**
	 * 从世界移除 AI 机器人实体（宕机 / 自爆 / 玩家离开）
	 * 增量渲染下需显式发送 delete 包，通知客户端停止跟踪并释放缓存
	 * @param {import('./robot/RobotEntity.js').default} robot
	 */
	removeRobot(robot) {
		const idx = this.entities.indexOf(robot);
		if (idx !== -1) this.entities.splice(idx, 1);
		this.markEntityRemoved({ id: robot.data.id });
	}
	...
	getNearbySpawnOutpost(px, py, team) {
		...
		for (const outpost of this.outposts) {
			if (
				outpost.state.owner === team &&
				outpost.isWithinRange(px, py, SPAWN_SET_RADIUS)
			) {
				return outpost;
			}
		}
