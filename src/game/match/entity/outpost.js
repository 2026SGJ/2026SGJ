...
class Outpost extends Entity {
	...
	/**
	 * AI 机器人拆塔：对敌方前哨站造成破坏（扣除占领进度）
	 *
	 * 无人机拆前哨站约需 28 秒：进度 200 ÷（每发 2 点 × 4 发/秒）≈ 25 秒。
	 * 中立前哨站不受影响（仅敌方占领中的前哨站可被拆除）。
	 *
	 * @param {'A'|'B'} team - 攻击方队伍
	 * @param {number} amount - 每发扣除的进度
	 */
	drainByRobot(team, amount) {
		if (this.state.owner === null || this.state.owner === team) return;
		...
		if (this.state.progress <= 0) {
			...
			this.state.spawnSet = {};
			console.log(
				`[Outpost] ${this.data.id} 被队伍 ${team} 的机器人拆毁，恢复中立`,
			);
		}
	}
	...
	tickCapture(players) {
		...
		if (this.state.owner === null) {
			...
			if (this.state.progress >= CAPTURE_MAX) {
				...
				console.log(`[Outpost] ${this.data.id} 被队伍 ${activeTeam} 占领！`);
			}
			...
			if (this.state.progress <= 0) {
				...
				this.state.owner = null;
				this.state.progress = Math.min(CAPTURE_MAX, overflow);
				console.log(
