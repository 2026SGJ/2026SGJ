...
class ShopSession {
	 ...
	 * @param {Object} [context] - 购买上下文（机器人强化芯片使用，见 Shop.buy）
	 * @returns {{ok: boolean, reason?: string, item?: Object}} Shop 实体的购买结果
	 */
	buy(itemId, context) {
		const result = this.shop.buy(this.player, itemId, context);
		if (result.ok) {
			const data = {
				success: true,
				itemId,
			...
			// 词条抽奖券：回执中附带抽到的词条信息（供客户端展示）
			if (result.trait) {
				data.trait = result.trait;
			}
			sendBuyItem(this.player.sessionId, data);
			console.log(
	...
	hasItem(itemId) {
		...
		return [...catalog.permanent, ...catalog.refresh].some(
			(it) => it.id === itemId,
		);
	}
