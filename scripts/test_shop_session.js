...
const [{ default: ShopSession }, { default: ShopEntity }, { sent }] =
	await Promise.all([
...
check(
	openPkt.shop.id === "shop_base_A" && openPkt.shop.team === "A",
	"S2COpenShop 含商店信息",
);

const list1 = take("S2CShopList");
check(
	list1 && list1.permanent.length === 4,
	`S2CShopList 常驻商品 4 个（实际 ${list1?.permanent.length}）`,
);
check(
	list1 && list1.refresh.length === 4,
	`S2CShopList 刷新商品 4 个（实际 ${list1?.refresh.length}）`,
);
check(list1 && list1.money === 500, "S2CShopList 含玩家金钱");
check(
	list1 && list1.permanent[0].id === "shop_item_potion_health",
	"清单商品 id 正确",
);
...
check(
	buyPkt &&
		buyPkt.success === true &&
		buyPkt.itemId === "shop_item_potion_health",
	"S2CBuyItem 成功回执",
);
...
// ============================================================
player.money = 100;
// 刷新清单是随机抽取的（refresh 商品池随机选 4 个上架），不能硬编码商品 id；
// 从当前实际清单中选一个价格 > 100 的商品，确保命中「金钱不足」分支
const catA = shop.getCatalog("A");
const tooExpensive = [...catA.permanent, ...catA.refresh].find(
	(it) => it.price > player.money,
);
check(
	!!tooExpensive,
	`存在价格 > ${player.money} 的商品（refresh: ${catA.refresh.map((i) => i.id).join(",")}）`,
);
const buyFail = session.buy(tooExpensive.id);
check(
	buyFail.ok === false && buyFail.reason === "insufficient_money",
	"金钱不足购买失败",
);
...
check(
	failPkt &&
		...
		failPkt.reason === "insufficient_money",
	"S2CBuyItem 失败原因回执",
);
...
check(
	unknown.ok === false && unknown.reason === "not_in_shop",
	"未知道具购买失败（not_in_shop）",
);
...
// hasItem 路由判断
check(
	session.hasItem("shop_item_potion_health") === true,
	"hasItem 命中商店商品",
);
check(
	session.hasItem("bomb") === false,
	"hasItem 排除非商店商品（物品栏道具）",
);
...
check(
	player._shopSession === null && player._openShop === null,
	"close() 清理玩家引用",
);
...
check(
	list3 &&
		list3.refresh.length === 1 &&
		list3.refresh[0].id === "shop_item_armor_plate",
	"换货后推送最新清单",
);
